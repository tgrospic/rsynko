...
fn a_tweet_carrying_only_pictures_asks_for_one_by_name() {
    ...
    let photo = MediaSyntax.format(
        ...
        MediaSyntax.metadata([
            (FORMAT_SOURCE.to_owned(), MediaSyntax.string_metadata("https://pbs.twimg.com/media/A.jpg?name=orig")),
            (FORMAT_EXTENSION.to_owned(), MediaSyntax.string_metadata("jpg")),
    ...
    manager.apply_manager_event(ManagerIntentOp::FormatCatalog {
        id,
        event: FormatDiscoveryOp::Available { formats: vec![photo] },
    });
