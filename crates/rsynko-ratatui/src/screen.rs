...
pub(crate) enum RenderedBody {
    ...
    Rows { title: String, focused: Option<usize>, rows: Vec<Vec<RenderedLine>> },
    /// Holds one message standing in for content there is none of yet.
    Message { title: String, message: String, style: Style },
    /// Holds a text draft under edit, and the shapes of what may be written in it.
    Draft { title: String, placeholder: String, text: String, cursor: usize, examples: Vec<(String, String)> },
...
impl ScreenSyntax for RatatuiScreen {
    ...
    fn line(&self, runs: impl Iterator<Item = Self::Text>) -> Self::Line {
        RenderedLine { spans: runs.collect(), edit: None }
    }

    fn edited_line(&self, prefix: impl Into<String>, text: &str, cursor: usize, emphasis: Emphasis) -> Self::Line {
        RenderedLine {
            spans: Vec::new(),
            edit: Some(Edit { prefix: prefix.into(), text: text.to_owned(), cursor, style: style(emphasis) }),
        }
    }
    ...
    fn rows(
        ...
        RenderedBody::Rows { title: title.into(), focused, rows: rows.collect() }
    }

    fn message(&self, title: impl Into<String>, message: impl Into<String>, emphasis: Emphasis) -> Self::Body {
        RenderedBody::Message { title: title.into(), message: message.into(), style: style(emphasis) }
    }
    ...
    fn record(&self, title: impl Into<String>, notes: impl Iterator<Item = String>) -> Self::Body {
        RenderedBody::Record { title: title.into(), notes: notes.collect() }
    }
    ...
    fn screen(
        ...
        RenderedScreen { header: header.spans, body, status: status.into(), footer: footer.spans }
    }
    ...
    fn screen_text(&self, screen: &Self::Screen) -> impl Iterator<Item = String> {
        ...
        let body = match &screen.body {
            RenderedBody::Rows { title, rows, .. } => {
                [title.clone()].into_iter().chain(rows.iter().flatten().map(line_text)).collect::<Vec<_>>()
            }
            RenderedBody::Message { title, message, .. } => vec![title.clone(), message.clone()],
            RenderedBody::Draft { title, placeholder, text, examples, .. } => {
                [title.clone(), if text.is_empty() { placeholder.clone() } else { text.clone() }]
                    .into_iter()
                    .chain(examples.iter().map(|(shape, means)| format!("{shape}  {means}")))
                    .collect()
            }
            RenderedBody::Record { title, notes } => [title.clone()].into_iter().chain(notes.clone()).collect(),
            RenderedBody::Verbatim { text } => vec![text.clone()],
        };
        [header].into_iter().chain(body).chain([screen.status.clone(), footer])
    }
}
...
fn style(emphasis: Emphasis) -> Style {
    match emphasis {
        ...
        Emphasis::Name => Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD),
        Emphasis::Heading => Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD),
...
fn line_text(line: &RenderedLine) -> String {
    line.edit.as_ref().map_or_else(|| span_text(&line.spans), |edit| format!("{}{}", edit.prefix, edit.text))
}
...
pub(crate) fn paint(frame: &mut Frame<'_>, screen: &RenderedScreen, area: Rect) {
    ...
    .split(area);
    frame.render_widget(
        Paragraph::new(Line::from(screen.header.clone())).block(Block::default().borders(Borders::ALL)),
        areas[0],
    );
    paint_body(frame, &screen.body, areas[1]);
    frame.render_widget(
        Paragraph::new(Line::from(elided_runs(&screen.footer, inner_columns(areas[2]))))
            .block(Block::default().title(format!(" {} ", screen.status)).borders(Borders::ALL)),
        areas[2],
    );
}
...
fn elided_runs(runs: &[Span<'static>], columns: usize) -> Vec<Span<'static>> {
    ...
    runs.iter()
        .scan(columns.saturating_sub(1), |left, run| match run.content.width() {
            _ if *left == 0 => None,
            width if width <= *left => {
                *left -= width;
                Some(run.clone())
...
fn paint_body(frame: &mut Frame<'_>, body: &RenderedBody, area: Rect) {
    match body {
        RenderedBody::Rows { title, focused, rows } => paint_rows(frame, title, *focused, rows, area),
        RenderedBody::Message { title, message, style } => {
            frame.render_widget(
                Paragraph::new(Span::styled(message.clone(), *style)).block(titled(title)).wrap(Wrap { trim: true }),
                area,
            );
        }
        RenderedBody::Draft { title, placeholder, text, cursor, examples } => {
            paint_draft(frame, title, placeholder, text, *cursor, examples, area);
        }
        RenderedBody::Record { title, notes } => {
            let lines = notes.iter().map(|note| Line::styled(note.clone(), style(Emphasis::Muted))).collect::<Vec<_>>();
            frame
                .render_widget(Paragraph::new(Text::from(lines)).block(titled(title)).wrap(Wrap { trim: false }), area);
        }
        ...
        RenderedBody::Verbatim { text } => {
            frame.render_widget(Paragraph::new(Text::from(text.clone())).wrap(Wrap { trim: false }), area);
        }
    }
}
...
fn paint_rows(frame: &mut Frame<'_>, title: &str, focused: Option<usize>, rows: &[Vec<RenderedLine>], area: Rect) {
        ...
        for line in row {
            let drawn = match &line.edit {
                ...
                Some(edit) => {
                    ...
                    let (visible, column) = editor_window(&edit.text, edit.cursor, width.saturating_sub(prefix));
                    cursor =
                        Some((area.x.saturating_add(1).saturating_add(prefix).saturating_add(column), lines.len()));
    ...
    frame.render_widget(
        Paragraph::new(Text::from(lines)).block(titled(title)).scroll((u16::try_from(scroll).unwrap_or(u16::MAX), 0)),
        area,
    );
    if let Some((x, line)) = cursor
        ...
        && line - scroll < inner
    {
        let y = area.y.saturating_add(1).saturating_add(u16::try_from(line - scroll).unwrap_or(u16::MAX));
...
fn paint_draft(
    ...
    let (area, beneath) = if examples.is_empty() {
        ...
        let wanted = u16::try_from(examples.len()).unwrap_or(u16::MAX).saturating_add(2);
    ...
    frame.set_cursor_position(Position {
        ...
        y: area.y.saturating_add(1).saturating_add(cursor_row.saturating_sub(scroll)),
    });
}
...
fn paint_examples(frame: &mut Frame<'_>, examples: &[(String, String)], area: Rect) {
    let widest = examples.iter().map(|(shape, _)| shape.chars().count()).max().unwrap_or(0);
    let lines = examples
        .iter()
        .map(|(shape, means)| {
            Line::from(vec![
        ...
        .collect::<Vec<_>>();
    frame.render_widget(Paragraph::new(Text::from(lines)).block(titled("Examples")), area);
}
...
pub(crate) fn editor_window(text: &str, cursor: usize, width: u16) -> (String, u16) {
    ...
    let mut visible = if start > 0 { format!("…{}", &text[start..cursor]) } else { text[..cursor].to_owned() };
    let cursor_column = column_width(&visible);
    for character in text[cursor..].chars() {
        let next_width = UnicodeWidthStr::width(visible.as_str()).saturating_add(character.width().unwrap_or(0));
...
mod tests {
    ...
    fn a_menu_too_long_for_its_pane_ends_where_a_reader_can_see_it() {
        let runs = vec![Span::raw("[Esc] Transfers  ".to_owned()), Span::raw("[Ctrl+Q] Quit".to_owned())];
        ...
        let stated = cut.iter().map(|run| run.content.to_string()).collect::<String>();
    ...
    fn editor_cursor_uses_terminal_columns_and_wrapping() {
        ...
        assert_eq!(editor_window("long filename.mp4", "long filename.mp4".len(), 8), ("…me.mp4".to_owned(), 7));
    }
}
