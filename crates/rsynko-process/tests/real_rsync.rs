...
use rsynko_rsync::{
    RsyncEndpointExt, SYNC_PROGRAM, SyncCommandExt, SyncMode, SyncObservationViewAlg, SyncProfile, SyncProgramExt,
};
...
fn authored(root: &Path) {
    ...
    write(source.join("Invoices/2026/invoice-0104.pdf"), vec![7_u8; 2048]).expect("a kept file");
    write(source.join("Invoices/2026/invoice-0105.pdf"), vec![9_u8; 4096]).expect("a new file");
    ...
    write(backup.join("Invoices/2026/invoice-0104.pdf"), vec![7_u8; 2048]).expect("a kept file");
...
fn counted(changes: &[impl PlannedChangeAlg], kind: ChangeKind) -> usize {
    changes.iter().filter(|change| change.change_kind() == kind).count()
}
...
fn a_rehearsal_states_what_it_would_do_and_changes_nothing() {
    ...
    assert!(
        changes.iter().any(
            |change| change.change_path() == "Invoices/2026/invoice-0105.pdf" && change.change_size() == Some(4096)
        )
    );
    ...
    assert_eq!(read(backup.join("Notes/todo.txt")).expect("the old text"), b"the old text");
...
fn a_transfer_does_what_its_rehearsal_said_it_would() {
    ...
    let environment = ProcessSyncEnv::new(sender);
    let source = environment.read_endpoint(&format!("{}/", root.path().join("Documents").display()));
    let destination = environment.read_endpoint(&root.path().join("Documents-backup").display().to_string());
    ...
    let named = |changes: &[_]| {
        let mut paths = changes
            .iter()
            .map(|change: &_| {
                format!("{:?} {}", PlannedChangeAlg::change_kind(change), PlannedChangeAlg::change_path(change))
            })
            .collect::<Vec<_>>();
        paths.sort();
        paths
    ...
    let backup = root.path().join("Documents-backup");
    assert_eq!(read(backup.join("Invoices/2026/invoice-0105.pdf")).expect("the new file arrived"), vec![9_u8; 4096]);
    assert_eq!(read(backup.join("Notes/todo.txt")).expect("the changed file arrived"), b"the longer new text");
    assert!(!backup.join("Notes/gone.md").exists());
}
...
fn a_running_transfer_states_how_far_it_has_come() {
    ...
    for index in 0..4 {
        write(source.join(format!("part-{index}.bin")), vec![index; 512 * 1024]).expect("something worth watching");
    }
    let (sender, watched) = channel();
    let environment = ProcessSyncEnv::new(sender);
    let command = environment.transfer_command(
    ...
    let advanced =
        watched.try_iter().filter_map(|observation| environment.observation_progress(&observation)).collect::<Vec<_>>();
