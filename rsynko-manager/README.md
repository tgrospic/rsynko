...
- collection selection wraps and remains a stable identity — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- adding sources focuses the first newly appended identity — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- opening details denotes exactly the selected entry — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- expanded details move an ordered cursor through visible fields and actions — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- breadcrumbs are derived from page and entry identity — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- removing selection chooses a remaining neighbor or no selection for an empty collection — [`NavigationLaws::navigation_laws`](src/laws.rs#L83-L151);
- a draft is observed back exactly as it was stated, and submits the lines it names — [`DraftLaws::draft_laws`](src/laws.rs#L172-L229);
- Space schedules exactly one selected entry in `Ready` state — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- Space names a collection entry, so it denotes nothing on the editor and selector pages — [`MenuLaws::menu_laws`](src/laws.rs#L594-L664);
- Space pauses only active transfers that advertise cooperative pause support — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- unsupported, publishing, terminal, and failed transfers expose no pause action — [`MenuLaws::menu_laws`](src/laws.rs#L594-L664);
- disabled menu actions remain observable but cannot be dispatched by an interpreter — [`MenuLaws::menu_laws`](src/laws.rs#L594-L664);
- the first start irreversibly fixes request options — [`OptionsLaws::options_laws`](src/laws.rs#L1316-L1404);
- extracted titles prefill portable output names and fall back to media identity — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- text insertion, deletion, and cursor movement preserve UTF-8 boundaries — [`TextLaws::text_editor_laws`](src/laws.rs#L238-L269);
- failed requests restart without changing their fixed options — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- stream roles admit only formats with exactly the denoted audio/video composition — [`OptionsLaws::options_laws`](src/laws.rs#L1316-L1404);
- one ordered selector states the preferred roles and then every discovered format — [`OptionsLaws::options_laws`](src/laws.rs#L1316-L1404);
- preferring a role releases any fixed identity, and a fixed identity names exactly one format — [`OptionsLaws::options_laws`](src/laws.rs#L1316-L1404);
- duplication creates a fresh editable identity and transfer state — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- duplication preserves media and format choices — [`TransitionLaws::transition_laws`](src/laws.rs#L432-L546);
- every entry keeps the record of what was observed about it, keyed and in order — [`LogLaws::log_laws`](src/laws.rs#L747-L789);
- transfer observations update only their addressed [`ManagerSorts::Id`] — [`QueueLaws::queue_laws`](src/laws.rs#L308-L387);
- speed and estimated time are derived consistently for every interpreter — [`QueueLaws::queue_laws`](src/laws.rs#L308-L387);
- full byte progress denotes `Publishing`, not `Complete` — [`QueueLaws::queue_laws`](src/laws.rs#L308-L387);
- only terminal success denotes `Complete` — [`QueueLaws::queue_laws`](src/laws.rs#L308-L387);
- a collection holds exactly the sources it was given, in the order given — [`DownloadsLaws::downloads_laws`](src/laws.rs#L691-L723);
- stating what a transfer would do is not doing it — [`RehearsalLaws::rehearsal_laws`](src/laws.rs#L1008-L1121);
- a line nobody claimed is a path rather than something to fetch — [`SubmissionLaws::submission_laws`](src/laws.rs#L841-L938);
- what wants work, what has begun, and what is left holding are the same statement — [`AttentionLaws::attention_laws`](src/laws.rs#L1213-L1285);
- every intention denotes exactly the meaning derived for it — [`IntentLaws::intent_laws`](src/laws.rs#L1472-L1531);
